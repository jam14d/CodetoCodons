...
def app():
    ...
    # Title with cyberpunk glitch animation
    st.markdown("<h1 class='glitch-title'>Mutation Explorer</h1>", unsafe_allow_html=True)

    # Introduction Module
    st.markdown("""
    <div class='cyber-box'>
        <h3>Welcome to your **Neon-Lit Mutation Scanner**</h3>
        Analyze and visualize DNA mutations like a **hacker decoding genetic sequences**.
    ...
    if ref_file and var_file:
        ...
        st.sidebar.warning("⚠️ Upload Reference and Variant FASTA Files.")
        return
    ...
    # Visualizations
    st.subheader("Cybernetic Data Analysis")
    col1, col2 = st.columns(2)
    ...
    with col1:
        ...
        fig, ax = plt.subplots()
        ax.hist(snp_positions, bins=50, color='#FF3C00', alpha=0.9)
    ...
    with col2:
        ...
        snp_counts = {"Transitions": 60, "Transversions": 40}
        ax2.pie(snp_counts.values(), labels=snp_counts.keys(), autopct='%1.1f%%', colors=['#FF3C00', '#00FFC6'])
